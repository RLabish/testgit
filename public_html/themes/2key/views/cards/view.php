 ...
 $this->widget('zii.widgets.CDetailView', array(
	...
	'attributes'=>array(
        ...
        array(
        	'name'=>'ownerType',
 			'type'=>'raw',
        	'value'=>$model->ownerTypeNames[$model->ownerType],
         ),
		'owner',
 		'description',
        array(
        ...
        array(
        	'label'=>'Действует до',
 			'type'=>'raw',
        	'value'=>Yii::app()->DateFormatter->formatDateTime($model->expire,'medium',null),
         ), 		
 	),
)); 
...
foreach($fuels as $f) {
	echo '<tr class="'.(($i % 2 == 0)? 'even':'odd').'">';
	echo '<th>'.$f->name.'</th>';
	echo '<td class="col-volume col-1">'.$model->balanceByFuelId($f->id).'</td>';
	echo '<td></td>';
	echo '</tr>';
	$i++;	
}
