...
class DbHttpSession extends CDbHttpSession
{
    ...
    protected function createSessionTable($db, $tableName)
    {
        ...
        $db->createCommand()->addColumn($tableName, 'user_id', 'integer null');
        $db->createCommand()->addColumn($tableName, 'last_activity', 'datetime null');
        $db->createCommand()->addColumn($tableName, 'last_ip', 'string null');
    }
        ...
        try {
			...
			$db->createCommand()->update(
                    $this->userTableName, array(
                        'last_activity' => new CDbExpression('NOW()'),
                        'last_ip'       => CHttpRequest::getUserHostAddress(),
                    ),  'id=:user_id', array(':user_id' => Yii::app()->getUser()->getId())
                );
